	...
	class Mo_Ldap_Local_Save_Options_Handler {
		...
		public function mo_ldap_local_save_options() {
			...
			if ( isset( $_POST['option'] ) && current_user_can( 'manage_options' ) ) {
				...
				if ( strcmp( $post_option, 'mo_ldap_local_register_customer' ) === 0 && check_admin_referer( 'mo_ldap_local_register_customer' ) ) {
				...
				} elseif ( strcasecmp( $post_option, 'mo_ldap_local_trial_req' ) === 0 && check_admin_referer( 'mo_ldap_local_trial_req_nonce' ) ) {
					$email           = isset( $_POST['mo_ldap_local_trial_email'] ) ? sanitize_email( wp_unslash( $_POST['mo_ldap_local_trial_email'] ) ) : '';
					$site_type       = isset( $_POST['mo_ldap_local_site_type'] ) ? sanitize_text_field( wp_unslash( $_POST['mo_ldap_local_site_type'] ) ) : 'Single site';
					$plan_name       = isset( $_POST['mo_ldap_licensing_plan'] ) ? sanitize_text_field( wp_unslash( $_POST['mo_ldap_licensing_plan'] ) ) : 'Essential Authentication Plan';
					$description     = isset( $_POST['mo_ldap_local_description'] ) ? sanitize_text_field( wp_unslash( $_POST['mo_ldap_local_description'] ) ) : '';
					$alternate_email = isset( $_POST['mo_ldap_local_trial_alt_email'] ) ? sanitize_email( wp_unslash( $_POST['mo_ldap_local_trial_alt_email'] ) ) : '';

					$query = '';
					if ( ! empty( $plan_name ) ) {
						$query .= '<br><br>[Interested in plan] : ' . $plan_name;
					}

					if ( ! empty( $site_type ) ) {
						$query .= '<br><br>[Trial For] : ' . $site_type;
					}

					if ( ! empty( $description ) ) {
						$query .= '<br><br>[Requirements] : ' . $description;
					}

					if ( ! empty( $alternate_email ) ) {
						$query .= '<br><br>[Alternate Communication Email] : ' . $alternate_email;
					}

					$query = ' [Trial: WordPress LDAP/AD Plugin]: ' . $query;
					$this->mo_ldap_send_query( $email, '', $query );
					...
					$query            = 'Query :' . $call_reason . '<br><br> Time Zone: ' . $time_zone . '<br> <br>Date: ' . $call_date . '<br> <br>Time: ' . $call_time . '<br><br>Current Version Installed : ' . MO_LDAP_LOCAL_VERSION . ' <br>';
					$company          = isset( $_POST['company'] ) ? sanitize_text_field( wp_unslash( $_POST['company'] ) ) : '';
